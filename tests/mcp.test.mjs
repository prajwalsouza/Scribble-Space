...
test('packaged MCP initializes and serves every tool with real saved images',async()=>{
 ...
 try{
  ...
  const modelPath=path.join(dir,'sample.glb');await writeFile(modelPath,zip['scene.glb']);const viewpoint={position:[40,25,17],target:[9,7,-35],fov:65};const model=await tool('open_sketch',{modelPath,viewpoint});assert.deepEqual(JSON.parse(new URL(model.structuredContent.url).searchParams.get('viewpoint')),viewpoint);assert.equal((await tool('open_sketch',{viewpoint:{position:[0,0,0],target:[0,0,0]}})).isError,true);assert.match(new URL(model.structuredContent.url).searchParams.get('model'),/\.glb$/);assert.equal((await tool('open_sketch',{modelPath:'relative.glb'})).isError,true);assert.equal((await tool('read_sketch',{id:'../../secret'})).isError,true);
