...
const files=unzipStore(await readFile(filename));const parse=p=>JSON.parse(new TextDecoder().decode(files[p]));const manifest=parse('manifest.json');if(manifest.format!=='sm3dl'||manifest.version!=='0.1.0')throw Error('Unsupported format.');
