...
await copyFile(path.join(root,'demo.html'),path.join(target,'index.html'));await writeFile(path.join(target,'release.json'),JSON.stringify({app:'scribble-space',version:'0.2.1',files:inventory},null,2));console.log(`Published ${files.length} static files to ${target}`);
