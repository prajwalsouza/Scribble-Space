...
import {SubmissionQueue} from './lib/submissions.mjs';
...
export async function createEditorServer({dataDir=process.env.SCRIBBLE_DATA_DIR||path.join(ROOT,'data'),mcp=false}={}){
 const store=new ShotStore(dataDir);await store.init();
 const submissions=new SubmissionQueue(store);if(mcp)await submissions.init();
 const server=http.createServer(async(req,res)=>{
  ...
  try{
   ...
   const sessionQuery=new URL(req.url,'http://localhost').searchParams.get('session');
   if(pathname==='/api/session'&&req.method==='GET'&&mcp){submissions.requireSession(sessionQuery);return json(res,{active:true})}
   if(pathname.startsWith('/api/submissions/')&&req.method==='GET'&&mcp)return json(res,submissions.receipt(pathname.slice(17),sessionQuery));
 ...
 });return {server,store,submissions};
}
